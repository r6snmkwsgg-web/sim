...
const P = 92;                // one segment: 40 bookcases (80 m) + a rest area (12 m) — "every three hundred feet or so"
...
const START_FLOOR = 7302118441, START_SEG = 311207;
...
function lookHash(f, c, side, r, p) {
  let h = mix((lo32(f) ^ 0x51ed270b) >>> 0);
  h = mix((h ^ hi32(f)) >>> 0);
  h = mix((h ^ lo32(c)) >>> 0);
  h = mix((h ^ hi32(c) ^ (side ? 0xa5a5a5a5 : 0x2545f491)) >>> 0);
  h = mix((h ^ ((r * 131 + p * 7919) >>> 0)) >>> 0);
  return h;
}
function bookLook(id) {
  const h = lookHash(id.f, id.c, id.s, id.r, id.p), pc = PALETTE[h % PALETTE.length];
  ...
  return {
    ...
    band: st % 3, label: (st >>> 4) % 3 === 0 ? 1 : 0, wear: ((st >>> 8) & 255) / 255, out: ((st >>> 16) & 255) / 255 * 0.014
  };
}
...
function textHash(id, salt) { let h = mix(((salt * 0x2545F491) >>> 0) ^ 0x1234567); h = hnum(h, id.f); h = hnum(h, id.c * 2 + id.s); return mix((h ^ (id.r * 131 + id.p * 7919)) >>> 0); }
...
const keyOf = id => id.s + ':' + id.f + ':' + id.c + ':' + id.r + ':' + id.p;
const parseKey = k => { const a = k.split(':').map(Number); return { s: a[0], f: a[1], c: a[2], r: a[3], p: a[4] }; };
const sameId = (a, b) => a && b && a.s === b.s && a.f === b.f && a.c === b.c && a.r === b.r && a.p === b.p;
...
/* ==========================================================================
   Saved state
   ========================================================================== */
const SAVE_KEY = 'short-stay-in-hell-v2';
let S = null;
function freshState() {
  return {
    v: 2, name: 'Soren Johansson', act: 1, year: 1, day: 1, time: 7.25,
    homeSeg: START_SEG, homeFloor: START_FLOOR, side: 0,
    seg: START_SEG, lx: 86.5, floor: START_FLOOR, ly: 0, z: -4.6, yaw: Math.PI * 0.92, pitch: -0.04,
...
function loadSave() { try { const t = localStorage.getItem(SAVE_KEY); if (!t) return null; const s = JSON.parse(t); return s && s.v === 2 ? s : null; } catch (e) { return null; } }
...
function recountOver() { overFloors = new Map(); for (const k in S.over) { const a = k.split(':'); const key = a[0] + ':' + a[1]; overFloors.set(key, (overFloors.get(key) || 0) + 1); } }
