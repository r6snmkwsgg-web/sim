...
function renderJournal() {
  ...
  else if (jTab === 'frags') h = S.frags.length ? `<div class="frags">${S.frags.map(f => `<blockquote><q>${esc(f.text)}</q><cite>${esc(addrLine(parseKey(f.addr)))} · page ${f.page} · day ${f.day}</cite></blockquote>`).join('')}</div>` : '<p class="note" style="max-width:52ch">No readable words yet. Most pages are noise from edge to edge. When a sentence surfaces, it is recorded here.</p>';
  else if (jTab === 'notes') {
    const ns = S.notes || [];
