"""The Flooded Stacks: rows of bookcases standing in knee-deep water under a grid of skylights.
Pink tile, turquoise water; the bottom shelves are under the surface."""
from lib import *
...
def make():
    ...
    # the flood: everything but a walkway round the walls
    ...
    for x in (4.0, 12.0, 20.0, 28.0):
        for (y, a) in ((py0 + 0.01, 0), (py1 - 0.01, 0)):
            R.light(box(x - 0.2, y - 0.04, -0.36, x + 0.2, y + 0.04, -0.2, 'e_pool'))
    # the stacks: double-sided bookcases in the water, a cross-aisle down the middle
    zb = -depth
    ...
    # walkers: the dry walkway round the edge, and the wet cross-aisle
    ...
    R.meta['label'] = 'The Flooded Stacks'
